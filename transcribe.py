#!/usr/bin/env python3
"""
Transcription script using WhisperX with speaker diarization.
Outputs transcription as a markdown file with speaker labels.
"""
...
def transcribe_audio(audio_path, output_path=None, language=None, device="cuda", model_size="large-v3"):
    """
    ...
    Args:
        ...
        model_size (str): WhisperX model size ('tiny', 'base', 'small', 'medium', 'large', 'large-v2', 'large-v3')
    ...
    # Load WhisperX model with better compute type
    print(f"Loading WhisperX model: {model_size}")
    compute_type = "float16" if device == "cuda" else "float32"
    model = whisperx.load_model(model_size, device, compute_type=compute_type)
    
    # Transcribe audio with better parameters
    ...
    # Diarize using pyannote.audio with better model
    print("Performing speaker diarization...")
    hf_token = load_hf_token()
    
    if hf_token:
        try:
            # Use the latest and most robust diarization model
            ...
            # Run diarization with better parameters
    ...
    if output_path is None:
        ...
        output_path = f"{audio_name}_transcript.md"
...
def main():
    parser = argparse.ArgumentParser(description="Transcribe audio using WhisperX with speaker diarization")
